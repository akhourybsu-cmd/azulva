...
export async function loadClickAnalytics(): Promise<{
  ...
  const { data, error } = await supabase
    ...
    .select("id, deal_id, source_id, destination_id, outbound_url, affiliate_url, user_id, created_at")
    .order("created_at", { ascending: false })
    .limit(500);
  if (error || !data) {
    return { total: 0, bySource: [], byDeal: [], byDestination: [], signedInVsAnon: { signed_in: 0, anon: 0 }, recent: [] };
  }
